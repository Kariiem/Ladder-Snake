...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_EditCard]="images\\Menu_Editcard.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_NewGame] = "images\\Menu_Newgame.jpg";
	MenuItemImages[ITM_Exit] = "images\\Menu_Exit.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	CellPosition Iterator = fromCell ;
	while( Iterator.VCell() > toCell.VCell() ){
		int x = GetCellStartX(Iterator) ;
		int y = GetCellStartY(Iterator) ;
		pWind->DrawLine(x+UI.LadderXOffset,y,x+UI.CellWidth-UI.LadderXOffset,y,FRAME);
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	int yabove = y1 - 0.65*yChange ;
	int aboveYabove = y1 - 1.4*yChange ;
